	...
	switch ((lo >> 3) & 7)
	{
		...
		case 5:
			...
			if (pm & 0x8000)
				...
				sprintf (buf, "($%04X,A%d)", pm, reg);
			break;
		case 6:
			PARAM_WORD (pm);
			temp = pm & 0xff;
			if (temp & 0x80)
				sprintf (buf, "(-$%02X,A%d,D%d.%c)", -(signed char)temp & 0xff, reg, (pm >> 12) & 7, (pm & 800) ? 'L' : 'W');
			else
				sprintf (buf, "($%02X,A%d,D%d.%c)", temp, reg, (pm >> 12) & 7, (pm & 800) ? 'L' : 'W');
			break;
		case 7:
			switch (reg)
			{
				...
				case 2:
					...
					if (pm & 0x8000)
						...
						sprintf (buf, "($%04X,PC)", pm);
					break;
				case 3:
					PARAM_WORD (pm);
					temp = pm & 0xff;
					if (temp & 0x80)
						sprintf (buf, "(-$%02X,PC,D%d.%c)", -(signed char)temp & 0xff, (pm >> 12) & 7, (pm & 800) ? 'L' : 'W');
					else
						sprintf (buf, "($%02X,PC,D%c.%c)", temp, (pm >> 12) & 7, (pm & 800) ? 'L' : 'W');
					break;
				case 4:
					if (size == 1)
					{
...
int Dasm68000 (unsigned char *pBase, char *buffer, int pc)
{
	...
	switch (op & 0xffc0)
	{
		case 0x0000:	
			PARAM_WORD(pm); 
			if (lo == 0x3c)
				sprintf (buffer, "ORI      #$%02X,CCR", pm & 0xff);
			else
			{
				ea = MakeEA (lo, p, 1, &count); p += count;
				sprintf (buffer, "ORI.B    #$%02X,%s", pm & 0xff, ea);
			}
			break;
		case 0x0040:	
			PARAM_WORD(pm); 
			if (lo == 0x3c)
				sprintf (buffer, "ORI      #$%04X,SR", pm & 0xffff);
			else
			{
				ea = MakeEA (lo, p, 2, &count); p += count;
				sprintf (buffer, "ORI.W    #$%04X,%s", pm & 0xffff, ea);
			}
			break;
		case 0x0080:	
			PARAM_LONG(pm); ea = MakeEA (lo, p, 4, &count); p += count;
			sprintf (buffer, "ORI.L    #$%08X,%s", pm, ea);
			break;
		case 0x0100: case 0x0300: case 0x0500: case 0x0700: case 0x0900: case 0x0b00: case 0x0d00: case 0x0f00:
			if ((lo & 0x38) == 0x08)
			{
				PARAM_WORD(pm);
				sprintf (buffer, "MOVEP.W  ($%04X,A%d),D%d", pm, rlo, rhi);
			}
			else
			{
				ea = MakeEA (lo, p, 2, &count); p += count;
		...
		case 0x0140: case 0x0340: case 0x0540: case 0x0740: case 0x0940: case 0x0b40: case 0x0d40: case 0x0f40:
			...
			{
				PARAM_WORD(pm);
				sprintf (buffer, "MOVEP.L  ($%04X,A%d),D%d", pm, rlo, rhi);
			}
			else
			{
				ea = MakeEA (lo, p, 2, &count); p += count;
		...
		case 0x0180: case 0x0380: case 0x0580: case 0x0780: case 0x0980: case 0x0b80: case 0x0d80: case 0x0f80:
			...
			{
				PARAM_WORD(pm);
				sprintf (buffer, "MOVEP.W  D%d,($%04X,A%d)", rhi, pm, rlo);
			}
			else
			{
				ea = MakeEA (lo, p, 2, &count); p += count;
		...
		case 0x01c0: case 0x03c0: case 0x05c0: case 0x07c0: case 0x09c0: case 0x0bc0: case 0x0dc0: case 0x0fc0:
			...
			{
				PARAM_WORD(pm);
				sprintf (buffer, "MOVEP.L  D%d,($%04X,A%d)", rhi, pm, rlo);
			}
			else
			{
				ea = MakeEA (lo, p, 2, &count); p += count;
		...
		case 0x0200:	
			PARAM_WORD(pm);
			if (lo == 0x3c)
				sprintf (buffer, "ANDI     #$%02X,CCR", pm & 0xff);
			else
			{
				ea = MakeEA (lo, p, 1, &count); p += count;
				sprintf (buffer, "ANDI.B   #$%02X,%s", pm & 0xff, ea);
			}
			break;
		case 0x0240:	
			PARAM_WORD(pm); 
			if (lo == 0x3c)
				sprintf (buffer, "ANDI     #$%04X,SR", pm & 0xffff);
			else
			{
				ea = MakeEA (lo, p, 2, &count); p += count;
				sprintf (buffer, "ANDI.W   #$%04X,%s", pm & 0xffff, ea);
			}
			break;
		case 0x0280:	
			PARAM_LONG(pm); ea = MakeEA (lo, p, 4, &count); p += count;
			sprintf (buffer, "ANDI.L   #$%08X,%s", pm, ea);
			break;
		case 0x0400:	
			PARAM_WORD(pm); ea = MakeEA (lo, p, 1, &count); p += count;
			sprintf (buffer, "SUBI.B   #$%02X,%s", pm & 0xff, ea);
			break;
		case 0x0440:	
			PARAM_WORD(pm); ea = MakeEA (lo, p, 2, &count); p += count;
			sprintf (buffer, "SUBI.W   #$%04X,%s", pm & 0xffff, ea);
			break;
		case 0x0480:	
			PARAM_LONG(pm); ea = MakeEA (lo, p, 4, &count); p += count;
			sprintf (buffer, "SUBI.L   #$%08X,%s", pm, ea);
			break;
		case 0x0600:	
			PARAM_WORD(pm); ea = MakeEA (lo, p, 1, &count); p += count;
			sprintf (buffer, "ADDI.B   #$%02X,%s", pm & 0xff, ea);
			break;
		case 0x0640:	
			PARAM_WORD(pm); ea = MakeEA (lo, p, 2, &count); p += count;
			sprintf (buffer, "ADDI.W   #$%04X,%s", pm & 0xffff, ea);
			break;
		case 0x0680:	
			PARAM_LONG(pm); ea = MakeEA (lo, p, 4, &count); p += count;
			sprintf (buffer, "ADDI.L   #$%08X,%s", pm, ea);
			break;
		case 0x0800:
			PARAM_WORD(pm); ea = MakeEA (lo, p, 2, &count); p += count;
			sprintf (buffer, "BTST     #$%02X,%s", pm & 0xff, ea);
			break;
		case 0x0840:
			PARAM_WORD(pm); ea = MakeEA (lo, p, 2, &count); p += count;
			sprintf (buffer, "BCHG     #$%02X,%s", pm & 0xff, ea);
			break;
		case 0x0880:
			PARAM_WORD(pm); ea = MakeEA (lo, p, 2, &count); p += count;
			sprintf (buffer, "BCLR     #$%02X,%s", pm & 0xff, ea);
			break;
		case 0x08c0:
			PARAM_WORD(pm); ea = MakeEA (lo, p, 2, &count); p += count;
			sprintf (buffer, "BSET     #$%02X,%s", pm & 0xff, ea);
			break;
		case 0x0a00:	
			PARAM_WORD(pm);
			if (lo == 0x3c)
				sprintf (buffer, "EORI     #$%02X,CCR", pm & 0xff);
			else
			{
				ea = MakeEA (lo, p, 1, &count); p += count;
				sprintf (buffer, "EORI.B   #$%02X,%s", pm & 0xff, ea);
			}
			break;
		case 0x0a40:	
			PARAM_WORD(pm);
			if (lo == 0x3c)
				sprintf (buffer, "EORI     #$%04X,SR", pm & 0xffff);
			else
			{
				ea = MakeEA (lo, p, 2, &count); p += count;
				sprintf (buffer, "EORI.W   #$%04X,%s", pm & 0xffff, ea);
			}
			break;
		case 0x0a80:	
			PARAM_LONG(pm); ea = MakeEA (lo, p, 4, &count); p += count;
			sprintf (buffer, "EORI.L   #$%08X,%s", pm, ea);
			break;
		case 0x0c00:	
			PARAM_WORD(pm); ea = MakeEA (lo, p, 1, &count); p += count;
			sprintf (buffer, "CMPI.B   #$%02X,%s", pm & 0xff, ea);
			break;
		case 0x0c40:	
			PARAM_WORD(pm); ea = MakeEA (lo, p, 2, &count); p += count;
			sprintf (buffer, "CMPI.W   #$%04X,%s", pm & 0xffff, ea);
			break;
		case 0x0c80:	
			PARAM_LONG(pm); ea = MakeEA (lo, p, 4, &count); p += count;
			sprintf (buffer, "CMPI.L   #$%08X,%s", pm, ea);
			break;
		case 0x0e00:
			PARAM_WORD(pm); ea = MakeEA (lo, p, 1, &count); p += count;
			if (pm & 0x0800)
			{
				if (pm & 0x8000)
					sprintf (buffer, "MOVES.B  A%d,%s", (pm >> 12) & 7, ea);
				else
					sprintf (buffer, "MOVES.B  D%d,%s", (pm >> 12) & 7, ea);
			}
			else
			{
				if (pm & 0x8000)
					sprintf (buffer, "MOVES.B  %s,A%d", ea, (pm >> 12) & 7);
				else
					sprintf (buffer, "MOVES.B  %s,D%d", ea, (pm >> 12) & 7);
			}
			break;
		case 0x0e40:
			PARAM_WORD(pm); ea = MakeEA (lo, p, 2, &count); p += count;
			...
			{
				if (pm & 0x8000)
					sprintf (buffer, "MOVES.W  A%d,%s", (pm >> 12) & 7, ea);
				else
					sprintf (buffer, "MOVES.W  D%d,%s", (pm >> 12) & 7, ea);
			}
			else
			{
				if (pm & 0x8000)
					sprintf (buffer, "MOVES.W  %s,A%d", ea, (pm >> 12) & 7);
				else
					sprintf (buffer, "MOVES.W  %s,D%d", ea, (pm >> 12) & 7);
			}
			break;
		case 0x0e80:
			...
			{
				if (pm & 0x8000)
					sprintf (buffer, "MOVES.L  A%d,%s", (pm >> 12) & 7, ea);
				else
					sprintf (buffer, "MOVES.L  D%d,%s", (pm >> 12) & 7, ea);
			}
			else
			{
				if (pm & 0x8000)
					sprintf (buffer, "MOVES.L  %s,A%d", ea, (pm >> 12) & 7);
				else
					sprintf (buffer, "MOVES.L  %s,D%d", ea, (pm >> 12) & 7);
			}
			break;
		...
		case 0x4e40:
			...
			{
				PARAM_WORD (pm);
				sprintf (buffer, "LINK     A%d,#$%02X", rlo, pm);
			}
			else if ((lo & 0x38) == 0x18)
			{
				PARAM_WORD (pm);
				sprintf (buffer, "UNLK     A%d,#$%02X", rlo, pm);
			}
			...
			else if (lo == 0x3a)
			{
				...
				if (pm & 0x8000)
					sprintf (buffer, "MOVEC    %s,A%d", ea, (pm >> 12) & 7);
				else
					sprintf (buffer, "MOVEC    %s,D%d", ea, (pm >> 12) & 7);
			}
			else if (lo == 0x3b)
			{
				PARAM_WORD (pm);
				...
				if (pm & 0x8000)
					sprintf (buffer, "MOVEC    A%d,%s", (pm >> 12) & 7, ea);
				else
					sprintf (buffer, "MOVEC    D%d,%s", (pm >> 12) & 7, ea);
			}
			else
		...
		case 0x51c0: case 0x53c0: case 0x55c0: case 0x57c0: case 0x59c0: case 0x5bc0: case 0x5dc0: case 0x5fc0:
			if ((lo & 0x38) == 0x08)
			{
				...
				if (pm & 0x8000)
					...
					sprintf (buffer, "DB%s     D%d,*+$%04X     [%08X]", ccodes[(op >> 8) & 15], rlo, pm - 2, pc + pm + 2);
			}
			else
			{
				ea = MakeEA (lo, p, 1, &count); p += count;
		...
		case 0x6000: case 0x6040: case 0x6080: case 0x60c0:
			...
			if (pm == 0)
			{
				...
				if (pm & 0x8000)
					...
					sprintf (buffer, "BRA      *+$%04X     [%08X]", pm + 2, pc + pm + 2);
			}
			else
			{
				if (pm & 0x80)
					...
					sprintf (buffer, "BRA.S    *+$%02X       [%08X]", pm + 2, pc + pm + 2);
			}
			break;
		case 0x6100: case 0x6140: case 0x6180: case 0x61c0:
			...
			if (pm == 0)
			{
				...
				if (pm & 0x8000)
					...
					sprintf (buffer, "BSR      *+$%04X     [%08X]", pm + 2, pc + pm + 2);
			}
			else
			{
				if (pm & 0x80)
					...
					sprintf (buffer, "BSR.S    *+$%02X       [%08X]", pm + 2, pc + pm + 2);
			}
			break;
		...
		case 0x6e00: case 0x6e40: case 0x6e80: case 0x6ec0: case 0x6f00: case 0x6f40: case 0x6f80: case 0x6fc0:
			...
			if (pm == 0)
			{
				...
				if (pm & 0x8000)
					...
					sprintf (buffer, "B%s      *+$%04X     [%08X]", ccodes[(op >> 8) & 15], pm + 2, pc + pm + 2);
			}
			else
			{
				if (pm & 0x80)
					...
					sprintf (buffer, "B%s.S    *+$%02X       [%08X]", ccodes[(op >> 8) & 15], pm + 2, pc + pm + 2);
			}
			break;
		...
		case 0x7e00: case 0x7e40: case 0x7e80: case 0x7ec0:
			...
			if (pm & 0x80)
				...
				sprintf (buffer, "MOVEQ    #$%02X,D%d", pm, rhi);
			break;
