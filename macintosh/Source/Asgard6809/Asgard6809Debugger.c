...
//###################################################################################################
//
//	Asgard6809MiniTrace -- update the debugger
//
...
void Asgard6809MiniTrace(unsigned long inXY, unsigned long inSU, unsigned long inPCAB, unsigned long inDPFlags, int inICount)
{
	...
	if (sStateDumpActive)
	{
		...
		Dasm6809(tempString, inPCAB >> 16);
		
		// print it, along with the current state, to the file
		fprintf(sStateDumpLog[cpu], "%6d %08X %08X %04X %08X %s\n", inICount, inXY, inSU, inDPFlags, inPCAB, tempString);
		sStateDumpCount[cpu]++;
	}
	...
	if (sStateIOCountDown != 0)
	{
		...
		if (!--sStateIOCountDown)
		{
			...
			if (sStateSaveLog)
			{
				...
				tempState.fXY = inXY;
				tempState.fSU = inSU;
				tempState.fDPFlags = (unsigned short)inDPFlags;
				...
				if (fread(&tempState, 1, sizeof(tempState), sStateCompareLog) == sizeof(tempState))
				{
						...
						tempState.fDPFlags != (unsigned short)inDPFlags)
					{
						...
						for (i = 0; i < 4; i++)
						{
							...
							Dasm6809(tempString, tempState.fPCAB >> 16);
							printf("%6d XY=%08X SU=%08X DPFLAGS=%04X PCAB=%08X %s\n", 
									tempState.fICount, tempState.fXY, tempState.fSU, tempState.fDPFlags, tempState.fPCAB, tempString);
						}
	...
	//
	Dasm6809(tempString, inPCAB >> 16);
	printf("%6d XY=%08X SU=%08X DPFLAGS=%04X PCAB=%08X %s\n", inICount, inXY, inSU, inDPFlags & 0xffff, inPCAB, tempString);
...
//###################################################################################################
//
//	Asgard6809DebugRead -- internal debugging memory read that handles tracking and logging
//
//###################################################################################################

int Asgard6809DebugRead(int inAddress)
{
	int val = A6809_READMEM(inAddress);
...
//###################################################################################################
//
//	Asgard6809DebugRead -- internal debugging memory write that handles tracking and logging
//
//###################################################################################################
...
void Asgard6809DebugWrite(int inAddress, int inValue)
{
	...
	A6809_WRITEMEM(inAddress, inValue);
}
