...
//###################################################################################################
//
//	Asgard8080MiniTrace -- update the debugger
//
...
void Asgard8080MiniTrace(unsigned long inPCAF, unsigned long inSPDE, unsigned long inHLBC, unsigned long inFlags, int inICount)
{
	...
	if (sStateDumpActive)
	{
		...
		Dasm8085(tempString, inPCAF >> 16);
		
		// print it, along with the current state, to the file
		fprintf(sStateDumpLog[cpu], "%6d %08X %08X %08X %08X %s\n", 
				inICount, inSPDE, inHLBC, inFlags, inPCAF, tempString);
		sStateDumpCount[cpu]++;
	}
				...
				if (fread(&tempState, 1, sizeof(tempState), sStateCompareLog) == sizeof(tempState))
				{
						...
						tempState.fICount  != inICount)
					{
						...
						for (i = 0; i < 4; i++)
						{
							...
							Dasm8085(tempString, tempState.fPCAF >> 16);
							printf("%6d SPDE=%08X HLBC=%08X FLAGS=%08X PCAF=%08X %s\n", 
									tempState.fICount, tempState.fSPDE, tempState.fHLBC,
									tempState.fFlags, tempState.fPCAF, tempString);
						}
...
//###################################################################################################
//
//	Asgard8080DebugRead -- internal debugging memory read that handles tracking and logging
//
//###################################################################################################

int Asgard8080DebugRead(int inAddress)
{
	int val = A8080_READMEM(inAddress);
...
//###################################################################################################
//
//	Asgard8080DebugRead -- internal debugging memory write that handles tracking and logging
//
//###################################################################################################
...
void Asgard8080DebugWrite(int inAddress, int inValue)
{
	...
	A8080_WRITEMEM(inAddress, inValue);
}
